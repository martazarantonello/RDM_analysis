...
def process_delays(incident_files, st_code, output_dir):
    ...
    for period_name, file_path in incident_files.items():
        ...
        # Remove rows where EVENT_TYPE is "C"
        delay_df = delay_df[delay_df["EVENT_TYPE"] != "C"]
...
def adjust_schedule_timeline(processed_schedule, processed_delays, st_code=None):
    ...
    for col in ['PFPI_MINUTES', 'INCIDENT_REASON', 'SECTION_CODE', 'EVENT_DATETIME', 'event_time', 'delay_day']:
    ...
    schedule_columns = list(schedule_df.columns) + [
        'ACTUAL_CALLS', 'PFPI_MINUTES', 'INCIDENT_REASON', 'SECTION_CODE', 'DELAY_DAY', 'EVENT_DATETIME',
